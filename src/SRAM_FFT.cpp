...
void arduinoFFT::Compute(VPtr<double, SPIRAMVAlloc> &vReal, VPtr<double, SPIRAMVAlloc> &vImag, uint16_t samples, uint8_t dir)
{
#warning("This method is deprecated and may be removed on future revisions.")
    Compute(vReal, vImag, samples, Exponent(samples), dir);
}
...
void arduinoFFT::Compute(VPtr<double, SPIRAMVAlloc> &vReal, VPtr<double, SPIRAMVAlloc> &vImag, uint16_t samples, uint8_t power, uint8_t dir)
{	// Computes in-place complex-to-complex FFT
    // Reverse bits
#warning("This method is deprecated and may be removed on future revisions.")
    uint16_t j = 0;
...
void arduinoFFT::ComplexToMagnitude(VPtr<double, SPIRAMVAlloc> &vReal, VPtr<double, SPIRAMVAlloc> &vImag, uint16_t samples)
{	// vM is half the size of vReal and vImag
#warning("This method is deprecated and may be removed on future revisions.")
    for (uint16_t i = 0; i < samples; i++) {
        vReal[i] = sqrt(sq(vReal[i]) + sq(vImag[i]));
...
void arduinoFFT::DCRemoval(VPtr<double, SPIRAMVAlloc> &vData, uint16_t samples)
{
    // calculate the mean of vData
#warning("This method is deprecated and may be removed on future revisions.")
    double mean = 0;
...
void arduinoFFT::Windowing(VPtr<double, SPIRAMVAlloc> &vData, uint16_t samples, uint8_t windowType, uint8_t dir)
{// Weighing factors are computed once before multiple use of FFT
// The weighing function is symetric; half the weighs are recorded
#warning("This method is deprecated and may be removed on future revisions.")
    double samplesMinusOne = (double(samples) - 1.0);
    for (uint16_t i = 0; i < (samples >> 1); i++) {
        ...
        if (dir == FFT_FORWARD) {
            vData[i] = (double) (vData[i] * weighingFactor);
            vData[samples - (i + 1)] = (double) (weighingFactor * vData[samples - (i + 1)]);
        }
        else {
            vData[i] = (double) (vData[i] / weighingFactor);
            vData[samples - (i + 1)] = (double) (vData[samples - (i + 1)] / weighingFactor);
        }
    }
}
...
void arduinoFFT::MajorPeak(VPtr<double, SPIRAMVAlloc> &f, VPtr<double, SPIRAMVAlloc> &v)
{
    double maxY = 0;
    uint16_t IndexOfMaxY = 0;
    //If sampling_frequency = 2 * max_frequency in signal,
...
double arduinoFFT::MajorPeak(VPtr<double, SPIRAMVAlloc> &vD, uint16_t samples, double samplingFrequency)
{
#warning("This method is deprecated and may be removed on future revisions.")
    double maxY = 0;
    uint16_t IndexOfMaxY = 0;
...
void arduinoFFT::MajorPeak(VPtr<double, SPIRAMVAlloc> &vD, uint16_t samples, double samplingFrequency, VPtr<double, SPIRAMVAlloc> &f, VPtr<double, SPIRAMVAlloc> &v)
{
#warning("This method is deprecated and may be removed on future revisions.")
    double maxY = 0;
    uint16_t IndexOfMaxY = 0;
...
void arduinoFFT::Swap(VPtr<double, SPIRAMVAlloc> &x, uint16_t i, uint16_t j)
{
    double temp = x[i];
    x[i] = x[j];
    x[j] = temp;
